<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $bilhete = $mysqli->real_escape_string($_POST["bilhete"]);
        $senha = md5($mysqli->real_escape_string($_POST["senha"]));
        //$senha_2 = password_hash($senha, PASSWORD_DEFAULT);

        $sql = "SELECT * FROM usuarios WHERE emailUsuario = '$email' AND nBilheteUsuario = '$bilhete'";
        $verify_sql = $mysqli -> query($sql);

        if (mysqli_num_rows($verify_sql) > 1) {

            echo "
                <script> window.alert('Email já existente, faça o login!') </script>
            ";

        } else {

            $stmt_sql = "INSERT INTO usuarios(nomeUsuario, emailUsuario, telemovelUsuario, nBilheteUsuario, senhaUsuario) VALUES ('$nome', '$email', '$telemovel', '$bilhete', '$senha')";
        
            $add_sql = $mysqli -> query($stmt_sql);

            echo "
                <script> window.alert('Usuario cadastrado com sucesso!') </script>
            ";
